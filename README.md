<div align="center">

# Hi, I'm Sashikanta Sahoo

### 🏥 Healthcare · Full-Stack · Feature-by-Feature Learning

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/sashikanta-sahoo-108688313)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/sashikantcodex)
[![Location](https://img.shields.io/badge/Bengaluru-India-2ea44f?style=for-the-badge&logo=googlemaps&logoColor=white)]()

<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=500&size=22&pause=1000&color=58A6FF&center=true&vCenter=true&width=600&lines=Software+Development+Engineer+@+IQVIA;Full-Stack+%7C+Python+%7C+Node+%7C+Next.js;Building+reliable+systems+in+Life+Sciences;Documenting+code+one+feature+at+a+time" alt="Typing intro" />

</div>

---

## ⚡ About Me

🏢 **Software Development Engineer 4** at [**IQVIA**](https://www.linkedin.com/company/iqvia) — building software that supports life-sciences and healthcare outcomes.

📍 **Bengaluru, India** · B.Tech in **Computer Science** (BPUT, Odisha)

🧭 **8+ years** across telecom, engineering services, health-tech consulting, and global pharma tech — from associate engineer to senior IC roles.

🌐 I design and ship **end-to-end web applications**: APIs, auth flows, React/Next.js frontends, and cloud-ready deployments.

📚 This profile is my **code workthrough** space — repos where I explore and document **one feature at a time** in Python, Node.js, and Next.js (not just demos — structured learning with real tooling).

🤖 Curious about **AI-assisted development**, auditability in regulated domains, and shipping faster without sacrificing clarity.

---

### 🚀 What I Bring to the Table

| Area | Focus |
|------|--------|
| **Domain** | Healthcare & life sciences, compliance-aware delivery |
| **Backend** | Node.js, REST APIs, authentication & authorization patterns |
| **Frontend** | React, Next.js, TypeScript, modern UI workflows |
| **Python** | Typed, tested learning projects (`src/` layout, CLI, CI-ready) |
| **Cloud & DevOps** | AWS fundamentals, CI/CD mindset, production hygiene |
| **Craft** | Readable PRs, small changes, tests where they matter |

---

### 🧠 Currently Exploring

- 🤖 AI-assisted coding with human-in-the-loop review
- 🔐 Audit logging & traceability in regulated environments
- ⚡ Next.js App Router patterns and server/client boundaries
- 🐍 Python typing, `pytest`, and professional project structure

---

## 🧪 Code Workthrough Labs

> Hands-on repos — each feature isolated, documented, and runnable.

| Repo | Stack | What you'll find |
|------|--------|------------------|
| [**python-all-features**](https://github.com/sashikantcodex/python-all-features) | Python | Lessons CLI, typing, tests, async, logging |
| [**NextJs-Features**](https://github.com/sashikantcodex/NextJs-Features) | TypeScript / Next.js | Modern frontend patterns & experiments |
| [**NodeJs-Auth-Features**](https://github.com/sashikantcodex/NodeJs-Auth-Features) | JavaScript / Node.js | Auth flows, middleware, API security basics |

---

## 🛠️ Tech Universe

### 🌐 Full-Stack & APIs

<p>
<img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white" alt="TypeScript"/>
<img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black" alt="JavaScript"/>
<img src="https://img.shields.io/badge/React-20232A?style=flat-square&logo=react&logoColor=61DAFB" alt="React"/>
<img src="https://img.shields.io/badge/Next.js-000000?style=flat-square&logo=nextdotjs&logoColor=white" alt="Next.js"/>
<img src="https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=nodedotjs&logoColor=white" alt="Node.js"/>
<img src="https://img.shields.io/badge/Express-000000?style=flat-square&logo=express&logoColor=white" alt="Express"/>
</p>

### 🐍 Python & Quality

<p>
<img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white" alt="Python"/>
<img src="https://img.shields.io/badge/pytest-0A9EDC?style=flat-square&logo=pytest&logoColor=white" alt="pytest"/>
<img src="https://img.shields.io/badge/Ruff-D7FF64?style=flat-square&logo=python&logoColor=black" alt="Ruff"/>
<img src="https://img.shields.io/badge/mypy-2B5B84?style=flat-square&logo=python&logoColor=white" alt="mypy"/>
</p>

### ☁️ Cloud & Delivery

<p>
<img src="https://img.shields.io/badge/AWS-232F3E?style=flat-square&logo=amazonaws&logoColor=white" alt="AWS"/>
<img src="https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white" alt="Docker"/>
<img src="https://img.shields.io/badge/GitHub_Actions-2088FF?style=flat-square&logo=githubactions&logoColor=white" alt="GitHub Actions"/>
<img src="https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white" alt="Git"/>
</p>

### 🗄️ Data & Tools

<p>
<img src="https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white" alt="PostgreSQL"/>
<img src="https://img.shields.io/badge/MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white" alt="MongoDB"/>
<img src="https://img.shields.io/badge/VS_Code-007ACC?style=flat-square&logo=visualstudiocode&logoColor=white" alt="VS Code"/>
</p>

---

## 📈 GitHub Analytics

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=sashikantcodex&show_icons=true&theme=tokyonight&hide_border=true&count_private=true" alt="GitHub stats" height="165"/>
<img src="https://github-readme-streak-stats.herokuapp.com/?user=sashikantcodex&theme=tokyonight&hide_border=true" alt="GitHub streak" height="165"/>

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=sashikantcodex&layout=compact&theme=tokyonight&hide_border=true" alt="Top languages" />

</div>

---

## 🗺️ Journey (snapshot)

```
2017 ── Associate Engineer @ Huawei (Telecom)
2018 ── Engineer @ L&T Technology Services
2020 ── Senior Software Engineer @ CitiusTech (Healthcare)
2024 ── SDE 4 @ IQVIA (Life Sciences)
```

---

## 🌱 Learning Philosophy

```text
Learn one feature → implement it → document it → ship a small PR.
Small diffs. Clear names. Tests when behavior matters.
```

---

## 🌐 Connect

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Sashikanta_Sahoo-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/sashikanta-sahoo-108688313)
[![GitHub](https://img.shields.io/badge/GitHub-sashikantcodex-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/sashikantcodex)

<br/>

**Open to collaborating on healthcare tech, full-stack projects, and clean learning repos.**

</div>

---

<div align="center">

### 💡 *Reliable software for regulated worlds. Curious code for everything else.*

<img src="https://komarev.com/ghpvc/?username=sashikantcodex&label=Profile%20views&color=58a6ff&style=flat" alt="Profile views" />

</div>
